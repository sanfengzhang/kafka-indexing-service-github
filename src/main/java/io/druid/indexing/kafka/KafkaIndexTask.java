...
public class KafkaIndexTask extends AbstractTask implements ChatHandler
{
  ...
  )
  {
    ...
    //endOffset的默认初始化值是Long.MAX_VALUE
  ...
  public TaskStatus run(final TaskToolbox toolbox) throws Exception
  {
    ...
    try (
      ...
      // Start up, set up initial offsets.初始化offset也是任务开试运行时候的offset
      ...
      if (restoredMetadata == null) {
        ...
        //这里初始化的nextOffset值就是从checkpoint中取出来的值
      ...
      try {
        while (stillReading) {
          ...
          for (ConsumerRecord<byte[], byte[]> record : records) {
            ...
            //消息记录的offset小于endOffsets
            if (record.offset() < endOffsets.get(record.partition())) {
            	//如果当前读取的记录的offSet位置和nextOffsets位置不相等则说明读取的
            	//偏移量不连续,默认如果不连续则会抛出异常
                ...
                for (InputRow row : rows) {
                  if (row != null && withinMinMaxRecordTime(row)) {
                    ...
                    if (addResult.isOk()) {
                      ...
                      //如果返回的segemnt的rows大于配置文件中的MaxRowsPerSegment,那么就将segment移出来，并创建一个新的segment
              ...
              //更新nextOffset的值，这样在下一次会继续对比读取的record位置、保证连续状态
              nextOffsets.put(record.partition(), record.offset() + 1);
            }
            //更新后的nextOffsets如果和endOffset相等则说明该消费者已经读到队列最后一条记录了，可以结束读取了
            //在这里会更新consumer的状态属性
      ...
      synchronized (statusLock) {
        ...
        status = Status.PUBLISHING;//当前任务状态转为发布状态,此时数据不能再更新或新增、并且开始准备发布
      }
  ...
  ) throws InterruptedException
  {
    ...
    if (offsets == null) {
      ...
      try {
        ...
        //这里会更新endOffset的值，在下一次任务中仍然初始化为Long.Max_VALUE
        if (finish) {
  ...
  private Set<Integer> assignPartitionsAndSeekToNext(KafkaConsumer consumer, String topic)
  {
	...
	//为consumer指定分区和开始消费的offset位置，这里offset的取值是nextOffset中的取值
	//会将nextoffset和endOffsets进行对比，来决定是否需要进行消费
